function generateKodeVerifikasi() {
    ...
    `;
        ...
        function exportDatabase() {
            ...
            if (users.length === 0) {
                alert('Tidak ada user untuk diekspor!');
                return;
            }
            ...
            reader.onload = function(e) {
                try {
                    ...
                    if (!data.users || !Array.isArray(data.users)) {
                        alert('❌ Format file backup rusak!');
                        return;
                    }
  ...
  `;
        ...
        function openUserManagement() {
            const currentUser = JSON.parse(localStorage.getItem('currentUser'));
            if (!currentUser || currentUser.role !== 'superadmin') {
                alert('Hanya Superadmin yang dapat mengakses!');
                return;
            }
  ...
  `;
        ...
        function perpanjangMasaAktif(username) {
            ...
            if (users[userIdx].role === 'superadmin' || users[userIdx].role === 'admin') {
                alert('SUPERADMIN & ADMIN tidak memiliki batasan masa aktif!');
                return;
            }
  ...
  `;
        ...
        function downloadMatchedPDF(matchedData, tujuan) {
            ...
            try {
                ...
                doc.text('LAPORAN DATA', 5, 10);

                // Judul tujuan - di posisi ~50mm (dengan spasi)
                doc.setTextColor(...titleColor);
                doc.text(tujuan.toUpperCase(), 52, 10); // spasi dari 5mm ke 52mm
                ...
                doc.text('TANGGAL : ' + tglStr, 5, 24);
                doc.text('JAM           : ' + jam, 5, 28);
                ...
                doc.text('TOTAL BELUM BAYAR : ' + belumBayar.length + ' PENUMPANG', 105, finalY + 5, {
                    align: 'center'
                });
    ...
    `).join('');
        ...
        async function downloadPDF() {
            ...
            try {
                ...
                doc.setFont("helvetica", 'bold');
                doc.setTextColor(255, 255, 255);
                doc.text('LAPORAN DATA PENUMPANG', 5, 10);
        ...
        function prosesGantiPassword() {
            const newPass1 = document.getElementById('newPassword1').value.trim();
            const newPass2 = document.getElementById('newPassword2').value.trim();

            if (!newPass1 || !newPass2) {
                alert('Masukkan password baru 2x!');
                return;
            }
            ...
            // Update password & reset masa berlaku
            const users = JSON.parse(localStorage.getItem('userDatabase') || '[]');
            const userIdx = users.findIndex(u => u.username === currentUser.username);

            if (userIdx !== -1) {
                users[userIdx].password = newPass1;
                users[userIdx].createdAt = new Date().toISOString(); // Reset masa berlaku
                localStorage.setItem('userDatabase', JSON.stringify(users));
            }
        ...
        async function autoProcessPDF() {
            ...
            try {
                ...
                if (matched.length === 0) {
                    alert("❌ TIDAK ADA MATCH di tabel data!");
                    return;
                }
